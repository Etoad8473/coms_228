...
public abstract class Living 
{
	...
	/**
	 * constructor for subclasses to override
	 */
	protected Living(Plain p, int r, int c) 
	{
		plain = p;
		row =r;
		column = c;
	...
	{		
		...
		//check the 9x9 array with a nested loop

		for(int x = -1; x <= 1; x++) 
		{
			for(int y = -1; y <= 1; y++) 
			{
				//	//skip if center tile -- BUG
				//if (!(x==0&&y==0))
				//{
					...
					//skip if outside plane
					if(!(checkRow < 0 || checkRow >= plain.getWidth()
						|| checkColumn < 0 || checkColumn >= plain.getWidth()))
					{
					
						//foreach adjacent tile
						Living adjacent = plain.grid[checkRow][checkColumn];
						
						//if state.badger then population[BADGER]++
						//else if chain of all the living object types
						if(adjacent.who() == State.BADGER)
							population[BADGER]++;
						else if(adjacent.who() == State.EMPTY)
							population[EMPTY]++;
						else if(adjacent.who() == State.FOX)
							population[FOX]++;
						else if(adjacent.who() == State.GRASS)
							population[GRASS]++;
						else if(adjacent.who() == State.RABBIT)
							population[RABBIT]++;
					}
				//}
			}
